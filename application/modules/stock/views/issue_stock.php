...
<div class="row">
	<div class="col-xs-12 col-sm-4 col-md-4 col-lg-4">
	  ...
	  <b>Issue To</b>
	   <?php $data=array('name' => 'issued_to','id'=> 'issued_to','class'=>'form-inline','required'=>'true'); echo form_input($data);?>
	    </div>
	</div>
	<div class="col-xs-12 col-sm-4 col-md-4 col-lg-4">
	  <div class="form-group">
	  <b>S11 #</b>
	    <?php $data=array('name' => 's11','id'=> 's11','class'=>'form-inline'); echo form_input($data);?>
	    </div>
	</div>
	<div class="col-xs-12 col-sm-4 col-md-4 col-lg-4">
	  <div class="form-group">
	   <b>Order </b>
	    <select name="order" class="form-inline" id="order">
...
// Add another row in the form on click add
		...
		$('#datepicker').datepicker({dateFormat: "yy-mm-dd"}).datepicker('setDate', null);
...
// Remove a row from the form
		   ...
		   var s11=retrieveFormValues('s11');
		   	...
		   	for(var i = 0; i < vaccine_count; i++) {
				...
				$.ajax(
			    {
			        url : formURL,
			        type: "POST",
			        data : {"transaction_type":get_transaction_type,"s11":get_s11, "date_issued":get_date_issued,"issued_to":get_issued_to,"vaccine":get_vaccine,"batch_no":get_batch,"expiry_date":get_expiry,"amt_issued":get_amt_issued,"vvm_status":get_vvm_status},
			       /* dataType : json,*/
    ...
    $('#issuestock_fm').validate({ // initialize the plugin
        rules: {
            issued_to: {
                required: true,
            },
